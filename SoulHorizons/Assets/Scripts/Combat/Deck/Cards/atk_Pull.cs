...
public class atk_Pull : AttackData
{
    ...
    public override void EndEffects(ActiveAttack activeAttack)
    {
        if(activeAttack.entityIsHit == true)
        {
           int entityXPos = activeAttack.entityHit._gridPos.x;
            ...
            if (entityXPos != xLimit)
            {
                entityXPos = activeAttack.entityHit._gridPos.x--;
                if (!scr_Grid.GridController.CheckIfOccupied(entityXPos, entityYPos) && (scr_Grid.GridController.ReturnTerritory(entityXPos, entityYPos).name == activeAttack.entityHit.entityTerritory.name))
                {
    ...
    int GetXLimit(int xPos, scr_Entity entity)
    {
